...
<p class="confirmation__paragraphe">Cher <span id="confirmPrenom">${firstName}</span> <span id="confirmNom">${lastName}</span>,
nous vous confirmons que votre commande de <span id="confirmPTotal">${totalPrice}</span>, est bien enregistrée. 
Vous recevrez un accusé de réception à l'adresse mail indiquée.</p>
<P class="confirmation__paragraphe">Pour suivre son acheminement, renseignez votre n° de commande : 
<span id="numCommande">${id}</span> lorsque vous contactez notre service après vente.</P>`

//On crée une fonction pour vider le panier. 
function clearCart(){
    let cartToClear = localStorage.getItem('panier');
    if (cartToClear != null) {
        localStorage.clear('panier');ls                     // ls???
    } 
}
clearCart();